...
Pattern Pilot is a **standalone, project-agnostic code quality control plane**. It orchestrates a review loop where Claude (code writer) and OpenAI (code reviewer) validate code against a target project's own governance rules.
...
- An automated QC gate that reviews code after Claude writes it
...
- An MCP server inside Claude Desktop — seamless, no context switching
...
- Receives `submit_for_review` calls from Claude Desktop
...
- Returns tiered findings to Claude
...
- OpenAI SDK (reviewer)
- Anthropic SDK (optional — for programmatic Claude calls in future)
...
│   │
│   └── mcp_server.py             # MCP entry point for Claude Desktop
│
├── tests/
...
|------|------|--------|--------------|
| Tier 1 | BLOCKING | Claude auto-fixes | Must resolve to exit loop |
| Tier 2a | recommended_autofix | Claude auto-fixes | Must resolve. Low-risk, local only |
...
1. Claude calls `submit_for_review(project_name, task_ref, files_changed)`
...
5. BLOCKING or recommended_autofix → return to Claude for fix → resubmit (round++)
6. PASS / PASS_WITH_ADVISORIES → log, exit
7. round > max_rounds with unresolved Tier 1 → REQUIRES_HUMAN_REVIEW → escalate

## Execution Model

- **v1: Synchronous.** Claude calls submit_for_review, Pattern Pilot blocks until full loop completes, returns verdict.
